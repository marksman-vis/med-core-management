...
const HMS = {
  currentUser: null,
  counters: { patient: 10, bill: 4, lab: 8, rx: 5, note: 0, appt: 10, user: 4, inv: 8, doctor: 8, staff: 8 },
  ...
  register(name, email, password, role, preventLogin = false, options = {}) {
    ...
    if(role === 'doctor') {
      let docId = 'D' + String(++this.counters.doctor).padStart(3, '0');
      ...
      let staffId = 'S' + String(++this.counters.staff).padStart(3, '0');
  ...
  saveData() {
    const data = {
      users: this.users, doctors: this.doctors, staff: this.staff,
      patients: this.patients, beds: this.beds, bills: this.bills,
      inventory: this.inventory, prescriptions: this.prescriptions,
      labResults: this.labResults, notes: this.notes,
      appointments: this.appointments, counters: this.counters
    };
    
    // Save to LocalStorage for backup
    localStorage.setItem('hms_full_backup', JSON.stringify(data));
    
    // Save to Server for permanent storage across all hosts
    fetch('/api/save', {
      method: 'POST',
...
(async function initData() {
  try {
    ...
    if (res.ok) {
      ...
      // Only merge if server has data
      if (serverData.patients && serverData.patients.length > 0) {
        Object.assign(HMS, serverData);
        console.log('HMS: Data synced from central server.');
      }
    }
  } catch (e) {
    console.warn('HMS: Server not responding, using local defaults.', e);
    // Fallback to localStorage if server is down
    const backup = localStorage.getItem('hms_full_backup');
    if (backup) Object.assign(HMS, JSON.parse(backup));
  }
